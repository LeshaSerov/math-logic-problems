...
public class ImplicationUsage {
            ...
            try {
                ...
                if (list.get(id).stream().count() > 2) {
                    if (list.get(id).get(1).equals("=>")) {
                        builder.append(outputFirstFunction).append(":").append(inputString).append("->").append(list.get(id).get(0));
                        builder.append("\n");
    ...
    public static void main(String[] args) {
        ImplicationUsage implicationUsage = new ImplicationUsage();
        implicationUsage.run();
    }
}
